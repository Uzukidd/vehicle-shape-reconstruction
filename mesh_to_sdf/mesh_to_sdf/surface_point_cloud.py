...
class SurfacePointCloud:
    ...
    def get_voxels(self, voxel_resolution, use_depth_buffer=False, sample_count=11, pad=False, check_result=False, return_gradients=False, bbox=None):
        result = self.get_sdf_in_batches(get_raster_points(voxel_resolution[0], voxel_resolution[1], voxel_resolution[2]), use_depth_buffer, sample_count, return_gradients=return_gradients, bbox=bbox)
        if not return_gradients:
            sdf = result
        else:
            sdf, gradients = result
            voxel_gradients = np.reshape(gradients, (voxel_resolution[0], voxel_resolution[1], voxel_resolution[2], 3))
