...
class SurfacePointCloud:
    ...
    def get_sdf(self, query_points, use_depth_buffer=False, sample_count=11, return_gradients=False, bbox=None):
        if use_depth_buffer:
            ...
            if bbox is not None:
                outbox_mask = np.zeros(query_points.shape[0])
                outbox_mask = np.any((query_points[:, 0] < bbox[3],
                                      query_points[:, 0] > bbox[4],
                                      query_points[:, 1] < bbox[5],
                                      query_points[:, 1] > bbox[6],
                                      query_points[:, 2] < bbox[7],
                                      query_points[:, 2] > bbox[8]
                                      ), axis=0)
                # (length, width, height, xmin, xmax, ymin, ymax, zmin, zmax)
                distances[outbox_mask] = 1.0
    ...
    def get_sdf_in_batches(self, query_points, use_depth_buffer=False, sample_count=11, batch_size=1000000, return_gradients=False, bbox=None):
        if query_points.shape[0] <= batch_size:
            return self.get_sdf(query_points, use_depth_buffer=use_depth_buffer, sample_count=sample_count, return_gradients=return_gradients, bbox=bbox)
    ...
    def get_voxels(self, voxel_resolution, use_depth_buffer=False, sample_count=11, pad=False, check_result=False, return_gradients=False, bbox=None):
        result = self.get_sdf_in_batches(get_raster_points(
            voxel_resolution[0], voxel_resolution[1], voxel_resolution[2]), use_depth_buffer, sample_count, return_gradients=return_gradients, bbox=bbox)
        if not return_gradients:
            sdf = result
        else:
            sdf, gradients = result
            voxel_gradients = np.reshape(
                gradients, (voxel_resolution[0], voxel_resolution[1], voxel_resolution[2], 3))
...
def create_from_scans(mesh, bounding_radius=1, scan_count=100, scan_resolution=400, calculate_normals=True):
    ...
    return SurfacePointCloud(mesh,
                             points=np.concatenate(
                                 [scan.points for scan in scans], axis=0),
                             normals=np.concatenate(
                                 [scan.normals for scan in scans], axis=0) if calculate_normals else None,
                             scans=scans
                             )
...
def sample_from_mesh(mesh, sample_point_count=10000000, calculate_normals=True):
    if calculate_normals:
        points, face_indices = mesh.sample(
            sample_point_count, return_index=True)
