...
from .utils import scale_to_unit_cube, scale_to_nonuniform_cube, scale_to_unit_sphere, get_raster_points, check_voxels
import trimesh
...
def mesh_to_voxels(mesh, voxel_resolution=[64, 64, 64], standard_bbox=None, bbox=None, surface_point_method='scan', sign_method='normal', scan_count=100, scan_resolution=400, sample_point_count=10000000, normal_sample_count=11, pad=False, check_result=False, return_gradients=False):
    mesh = scale_to_nonuniform_cube(mesh, bbox=standard_bbox)
    ...
    return surface_point_cloud.get_voxels(voxel_resolution, sign_method == 'depth', normal_sample_count, pad, check_result, return_gradients, bbox)
